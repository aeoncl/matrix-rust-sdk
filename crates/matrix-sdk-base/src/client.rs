...
use ruma::{
    ...
    events::{
        ...
        room::member::{
            MembershipState, OriginalRoomMemberEvent, OriginalSyncRoomMemberEvent,
...
impl BaseClient {
    ...
    pub async fn receive_members(
        ...
        let members: Vec<OriginalRoomMemberEvent> =
            response.chunk.iter().filter_map(|e| e.deserialize().ok()).collect();
        let mut ambiguity_cache = AmbiguityCache::new(self.store.clone());
